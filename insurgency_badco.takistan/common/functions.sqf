...
#define groupMembers(Y,Z)	([Y,Z] call group_Members)
...
nearest_Players = {
	...
	if (_type == "count") then { _result = 0; } else { _result = []; }; 
	{
		if (!isNil _x) then {
			_plr = call compile _x;
			if (!isNull _plr) then {
				if ((alive _plr || {!_alive}) && {_plr distance2d _pos <= _range}) then { 
...
#define nearestInfantry(W,X,Y,Z)	([W,X,Y,Z] call nearest_Infantry)
...
#define nearestMen2(W,X,Y,Z)	([W,X,Y,Z] call nearest_Men2)
...
#define knowsAboutAll(X,Y) ([X,Y] call knowsAbout_All)
