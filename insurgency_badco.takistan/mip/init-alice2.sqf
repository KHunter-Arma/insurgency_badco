...
switch toLower(worldName) do {		
        ...
        case "fallujah": {
                ...
                BIS_alice_mainscope setvariable ["spawnDistance",150, true];
        ...
        case "takistan": {
                BIS_alice_mainscope setvariable ["trafficDistance",2000];
                BIS_alice_mainscope setvariable ["spawnDistance",2000];
                BIS_alice_mainscope setVariable ["civilianCount","round (5 * (sqrt %1))"];
...
// Hunter'z civilian functions:

limit_civ_casualties = {
	
	if (side (_this select 1) == west) then {
	   
	westDeaths = westDeaths + 10;
        publicVariable "westDeaths";
        [-1, {hint "Civilian casualties are unacceptable.\nBLUFOR loses 10 respawns..." }] call CBA_fnc_globalExecute;
    // "-1" indicates all clients but not server
...
BIS_alice_mainscope setvariable ["ALICE_civilianinit",[
	...
	{
		// add handguns to 20% of the civilians
		if (random 1 > 0.8) then {
			if (random 1 > 0.7) then {
			   ...
			   {
                               [_this] joinSilent createGroup EAST;
				if ((random 1 > 0.3) && (random 1 < 0.7)) then  {
		...
		_this addEventHandler ["killed", {if (side (_this select 1) == west) then {
...
unused code that uses ws_assassins.sqf (civilian sleeper assassins) script

[BIS_alice_mainscope,"ALICE_civilianinit",[{nul = [true,"",(0+random 40),(0+random 40),west,1,0.7] execVM 'ws_assassins.sqf'}]] call BIS_fnc_variableSpaceAdd;
